...
export default function ChatPage() {
  ...
  const openGroup = (gruppo) => {
    setActiveChat({ type: 'group', id: gruppo, label: gruppo === 'generale' ? `🌊 ${t('tuttiGliAnimatori')}` : gruppo })
    setView('group')
  }
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {GRUPPI.filter(g => g === 'generale' || (profile?.tipi_animazione || []).includes(g) || profile?.ruolo !== 'animatore').map(gruppo => (
            <button key={gruppo} onClick={() => openGroup(gruppo)} className="card w-full p-4 flex items-center gap-3 active:scale-98 transition text-left">
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-116px)]">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-4 py-3 flex items-center gap-3">
        <button onClick={onBack} className="p-2 -ml-2 text-gray-600 hover:bg-gray-100 rounded-xl transition"><ChevronLeft size={22} /></button>
